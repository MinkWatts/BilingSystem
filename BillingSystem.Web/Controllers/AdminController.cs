...
namespace BillingSystem.Web.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> CreateUser(CreateUserDto model)
        {
            ...
            TempData["Error"] = "Failed to create user. Please check the details.";
        ...
        public async Task<IActionResult> EditUser(EditUserDto model)
        {
            ...
            model.UpdatedBy = User.FindFirstValue(ClaimTypes.Email) ?? "System";

            var result = await _mediator.Send(new EditUserCommand { UserDto = model });
        ...
        {
            model.CreatedBy = User.FindFirstValue(ClaimTypes.Email) ?? "System";
            await _mediator.Send(new AddCategoryCommand { CategoryDto = model });
        ...
        {
            model.CreatedBy = User.FindFirstValue(ClaimTypes.Email) ?? "System";
            await _mediator.Send(new AddSubCategoryCommand { SubCategoryDto = model });
        ...
        public async Task<IActionResult> DeleteCategory(int id)
        {
            ...
            if (category != null)
            {
                ...
                TempData["Success"] = "Category and its sub-categories removed.";
            }
        ...
        public async Task<IActionResult> AddProduct(CreateProductDto model)
        {
            if (model.SubCategoryId == 0)
            {
                ...
                TempData["Error"] = "Please select a valid sub-category.";
                return View(model);
            }

            model.CreatedBy = User.FindFirstValue(ClaimTypes.Email) ?? "System";
            var result = await _mediator.Send(new AddProductCommand { ProductDto = model });
